...
import {
  Mail, Lock, Eye, EyeOff, Phone, User, GraduationCap,
  ArrowRight, ArrowLeft, Check, ShieldCheck, Building2,
  AlertCircle, Sparkles, Hash, RefreshCw, ChevronDown,
  LogIn, UserPlus, X,
} from "lucide-react";
...
function AuthHeader({ icon: Icon, badge, title, sub }: { icon: React.ElementType; badge?: string; title: string; sub: string }) {
  return (
    <div className="px-8 pt-8 pb-6 border-b border-[rgba(27,58,107,0.08)]">
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <div>
          <span className="text-[11px] font-semibold tracking-[0.1em] uppercase text-[#5A6A8A]"
            style={{ fontFamily: "var(--font-mono)" }}>EduConnect</span>
          {badge && <span className="ml-2 px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-semibold rounded-full">{badge}</span>}
        </div>
      </div>
      <h1 className="text-[22px] text-[#0F1C3F]" style={{ fontFamily: "var(--font-serif)" }}>{title}</h1>
...
function PwdInput({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  ...
  return (
    <div className="relative">
      <Lock size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A]" />
      <input type={show ? "text" : "password"} value={value} onChange={e => onChange(e.target.value)}
        placeholder={placeholder ?? "Password"} className={`${inputCls} pl-9 pr-10`} />
...
function OTPRow({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const refs: React.RefObject<HTMLInputElement>[] = Array.from({ length: 6 }, () => ({ current: null }));
  const digits = value.split("").concat(Array(6).fill("")).slice(0, 6);
  const set = (i: number, v: string) => {
    const d = v.replace(/\D/, "").slice(-1);
    const nx = [...digits]; nx[i] = d;
    onChange(nx.join(""));
    ...
    <div className="flex gap-2 justify-center">
      {digits.map((d, i) => (
        <input key={i} ref={refs[i]} type="text" inputMode="numeric" maxLength={1}
          value={d} onChange={e => set(i, e.target.value)} onKeyDown={e => onKey(i, e)}
          className="w-11 h-12 text-center text-[18px] font-semibold rounded-xl outline-none transition-all"
          style={{ fontFamily: "var(--font-mono)", border: `2px solid ${d ? "#1B3A6B" : "#CBD5E1"}`, background: d ? "#EBF1FA" : "white" }} />
      ))}
    </div>
  );
}
...
export function StudentLogin({ onBack, onRegister, onSuccess }: {
  onBack: () => void; onRegister: () => void; onSuccess: () => void;
}) {
  ...
  const submit = () => {
    setError("");
    if (tab === "password") {
      if (!email || !pwd) { setError("Please fill in all required fields."); return; }
      setLoading(true);
  ...
  return (
    <AuthCard>
      <AuthHeader icon={GraduationCap} title="Student Sign In" sub="Access your career portal, roadmap, and placement dashboard." />
      ...
      <div className="px-8 py-7 space-y-5">
          ...
          <>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button className="text-[11.5px] text-[#1B3A6B] hover:underline font-medium">Forgot password?</button>
              </div>
            ...
            <label className="flex items-center gap-2.5 cursor-pointer select-none">
              ...
              <span className="text-[12.5px] text-[#5A6A8A]">Remember me for 30 days</span>
            </label>
          </>
        ) : (
          <>
            <div>
              ...
              <div className="flex gap-2">
                ...
                <div className="relative flex-1">
                  <Phone size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A]" />
                  <input type="tel" value={phone} onChange={e => setPhone(e.target.value.replace(/\D/, "").slice(0, 10))}
                    placeholder="10-digit mobile number" className={`${inputCls} pl-9`} />
                </div>
              </div>
            </div>
            {otpSent && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { setOtp(""); setOtpSent(false); }} className="text-[11.5px] text-[#1B3A6B] hover:underline">Resend</button>
                </div>
                <OTPRow value={otp} onChange={setOtp} />
                <p className="text-center text-[11.5px] text-[#5A6A8A]">OTP sent to +91 {phone}</p>
              </div>
            )}
          </>
        )}
        ...
        <button onClick={submit} disabled={loading}
          ...
          {loading ? "Signing in…" : tab === "otp" && !otpSent ? "Send OTP" : "Sign In"}
        </button>
        ...
        <p className="text-center text-[12.5px] text-[#5A6A8A]">
          New to EduConnect?{" "}
          <button onClick={onRegister} className="text-[#1B3A6B] font-semibold hover:underline">Create Account</button>
        </p>
      </div>

      <div className="px-8 pb-6">
        <button onClick={onBack} className="flex items-center gap-1.5 text-[12px] text-[#9AA5BE] hover:text-[#5A6A8A] transition-colors">
...
export function AdminLogin({ onBack, onSuccess }: { onBack: () => void; onSuccess: () => void }) {
  const [email, setEmail] = useState("");
  const [pwd, setPwd] = useState("");
  ...
  const submit = () => {
    setError("");
    if (!email || !pwd || !role) { setError("All fields are required."); return; }
    if (twoFA && otp.length < 6) { setError("Enter the 6-digit authenticator code."); return; }
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      if (!twoFA) { setTwoFA(true); }
      else { onSuccess(); }
    }, 1500);
  };
  ...
  return (
    <AuthCard>
      <AuthHeader icon={ShieldCheck} badge="Admin" title="Admin Sign In"
      ...
      <div className="mx-8 mt-6 flex items-start gap-2.5 px-3.5 py-3 bg-amber-50 border border-amber-200 rounded-xl">
        ...
        <p className="text-[12px] text-amber-700 leading-relaxed">
          This portal is for authorised personnel only. All login attempts are logged and monitored.
        </p>
      </div>
      ...
      <div className="px-8 py-7 space-y-5">
        ...
        {!twoFA ? (
          <>
            <div>
              ...
              <div className="relative">
                <ShieldCheck size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A] pointer-events-none" />
            ...
            {role === "College Admin" || role === "Placement Officer" || role === "Faculty Coordinator" ? (
              <div>
                <FieldLabel label="Institution" required />
                <div className="relative">
                  <Building2 size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A] pointer-events-none" />
                  <select className={`${inputCls} pl-9 appearance-none pr-8 cursor-pointer`}>
            ...
            <div>
              ...
              <div className="relative">
                <Mail size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A]" />
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@institution.edu"
                  className={`${inputCls} pl-9`} />
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <FieldLabel label="Password" required />
                <button className="text-[11.5px] text-[#1B3A6B] hover:underline font-medium">Reset password</button>
              </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {["256-bit SSL", "IP Allowlist", "Audit Log", "2FA Required"].map(b => (
                <span key={b} className="flex items-center gap-1 px-2 py-0.5 bg-[#EBF1FA] text-[#1B3A6B] text-[10.5px] font-medium rounded-full">
                  <Check size={9} />
                  {b}
                </span>
              ))}
          ...
          <div className="space-y-4">
            <div className="flex flex-col items-center gap-2 py-2">
              ...
              <p className="text-[14px] font-semibold text-[#0F1C3F]">Two-Factor Authentication</p>
            ...
            <OTPRow value={otp} onChange={setOtp} />
            <p className="text-center text-[11.5px] text-[#9AA5BE]">Code resets every 30 seconds</p>
            <button onClick={() => setTwoFA(false)} className="w-full text-center text-[12px] text-[#5A6A8A] hover:text-[#1B3A6B]">
              ← Back to credentials
            </button>
          </div>
        )}
        ...
        <button onClick={submit} disabled={loading}
          ...
          {loading ? (twoFA ? "Verifying…" : "Checking credentials…") : twoFA ? "Verify & Sign In" : "Continue to 2FA"}
        </button>

        <p className="text-center text-[11.5px] text-[#9AA5BE]">
          Having trouble?{" "}
          <button className="text-[#1B3A6B] hover:underline font-medium">Contact IT Support</button>
        </p>
      </div>
...
export function StudentRegister({ onBack, onLogin, onSuccess }: {
  onBack: () => void; onLogin: () => void; onSuccess: () => void;
}) {
  ...
  const next = () => {
    setError("");
    if (step === "details") {
      if (!name || !email || !phone || !pwd) { setError("Please fill all required fields."); return; }
      ...
      if (!emailVerified || !phoneVerified) { setError("Please verify both email and mobile."); return; }
      setStep("college");
    } else if (step === "college") {
      if (!college || !dept || !year) { setError("Please fill all required fields."); return; }
  ...
  return (
    <AuthCard>
      <AuthHeader icon={UserPlus} title="Create Student Account"
        ...
        {STEP_ORDER.filter(s => s !== "done").map((s, i) => {
          ...
          return (
            <div key={s} className="flex items-center flex-1 last:flex-none">
              ...
              {i < 2 && <div className={`flex-1 h-0.5 mx-1 transition-all ${done ? "bg-emerald-400" : "bg-[#EFF2FA]"}`} />}
            </div>
          );
        })}
      </div>
      ...
      <div className="px-8 py-5 space-y-4">
        ...
        {step === "details" && (
          <>
            <div>
              ...
              <div className="relative">
                <User size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A]" />
                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="As per college records"
                  className={`${inputCls} pl-9`} />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                <Mail size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A]" />
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="your.name@email.com"
                  className={`${inputCls} pl-9`} />
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                ...
                <div className="relative flex-1">
                  <Phone size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A]" />
                  <input type="tel" value={phone} onChange={e => setPhone(e.target.value.replace(/\D/, "").slice(0, 10))}
                    placeholder="10-digit number" className={`${inputCls} pl-9`} />
                </div>
              </div>
            </div>
            <div>
              <FieldLabel label="Create Password" required />
              <PwdInput value={pwd} onChange={setPwd} placeholder="Min. 8 characters" />
            </div>
            <div>
              <FieldLabel label="Confirm Password" required />
              <PwdInput value={confirmPwd} onChange={setConfirmPwd} placeholder="Re-enter password" />
              {confirmPwd && pwd !== confirmPwd && (
                <p className="text-[11.5px] text-red-500 mt-1 flex items-center gap-1"><X size={10} />Passwords do not match</p>
              )}
              {confirmPwd && pwd === confirmPwd && pwd.length >= 8 && (
                <p className="text-[11.5px] text-emerald-600 mt-1 flex items-center gap-1"><Check size={10} />Passwords match</p>
              )}
            </div>
        ...
        {step === "verify" && (
          <div className="space-y-5">
            {/* email otp */}
            <div className={`p-4 rounded-xl border-2 transition-all ${emailVerified ? "border-emerald-400 bg-emerald-50" : "border-[rgba(27,58,107,0.12)] bg-white"}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <Mail size={14} className={emailVerified ? "text-emerald-600" : "text-[#1B3A6B]"} />
                  <span className="text-[13px] font-semibold text-[#0F1C3F]">Verify Email</span>
                </div>
                {emailVerified
                  ? <span className="flex items-center gap-1 text-[11.5px] font-semibold text-emerald-600"><Check size={12} />Verified</span>
            ...
            {/* phone otp */}
            <div className={`p-4 rounded-xl border-2 transition-all ${phoneVerified ? "border-emerald-400 bg-emerald-50" : "border-[rgba(27,58,107,0.12)] bg-white"}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <Phone size={14} className={phoneVerified ? "text-emerald-600" : "text-[#1B3A6B]"} />
                  <span className="text-[13px] font-semibold text-[#0F1C3F]">Verify Mobile</span>
                </div>
                {phoneVerified
                  ? <span className="flex items-center gap-1 text-[11.5px] font-semibold text-emerald-600"><Check size={12} />Verified</span>
                  : <span className="text-[11px] text-[#5A6A8A]">+91 {phone}</span>}
              </div>
              {!phoneVerified && (
                <div className="space-y-2">
                  <OTPRow value={phoneOtp} onChange={setPhoneOtp} />
                  <div className="flex items-center justify-between">
                    <button onClick={() => verifyCode("phone")}
                      className="px-4 py-1.5 bg-[#1B3A6B] text-white text-[12px] font-semibold rounded-lg hover:bg-[#152d54] transition-colors">
                      Verify Code
                    </button>
                    <button className="text-[11.5px] text-[#1B3A6B] hover:underline">Resend</button>
        ...
        {step === "college" && (
          <>
            ...
            <div>
              ...
              <div className="relative">
                <GraduationCap size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A] pointer-events-none" />
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <div className="relative">
                  <Hash size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5A6A8A]" />
                  <input type="text" value={rollNo} onChange={e => setRollNo(e.target.value)} placeholder="Optional"
                    className={`${inputCls} pl-9`} />
                </div>
              </div>
            </div>
            ...
            <label className="flex items-start gap-2.5 cursor-pointer select-none pt-1">
              ...
              <span className="text-[12px] text-[#5A6A8A] leading-relaxed">
                I agree to the{" "}
                <button className="text-[#1B3A6B] font-medium hover:underline">Terms of Service</button>
                {" "}and{" "}
                <button className="text-[#1B3A6B] font-medium hover:underline">Privacy Policy</button>.
        ...
        {step === "done" && (
          <div className="flex flex-col items-center py-4 gap-4 text-center">
            ...
            <div>
              <h3 className="text-[18px] font-semibold text-[#0F1C3F]" style={{ fontFamily: "var(--font-serif)" }}>
                Welcome to EduConnect!
              </h3>
              <p className="text-[13px] text-[#5A6A8A] mt-1 leading-relaxed">
                Your account is ready. Start your AI-powered career journey right now.
              </p>
            </div>
            <div className="w-full space-y-2">
              {[
                ...
                <div key={i.text} className="flex items-center gap-2.5 px-4 py-2.5 bg-emerald-50 rounded-xl text-[12.5px] text-emerald-700 font-medium">
        ...
        <div className="flex items-center gap-3 pt-1">
          {step !== "details" && step !== "done" && (
            <button onClick={() => setStep(STEP_ORDER[curIdx - 1])}
          ...
          <button onClick={next} disabled={loading}
            ...
            {loading ? <RefreshCw size={15} className="animate-spin" /> : step === "done" ? <ArrowRight size={15} /> : <ArrowRight size={15} />}
            {loading ? "Please wait…" : step === "college" ? "Create Account" : step === "done" ? "Go to My Portal" : "Continue"}
          </button>
        </div>
        ...
        {step === "details" && (
          <p className="text-center text-[12.5px] text-[#5A6A8A]">
            ...
            <button onClick={onLogin} className="text-[#1B3A6B] font-semibold hover:underline">Sign In</button>
          </p>
        )}
      </div>
