...
// ─── VIDEO PLAYER ─────────────────────────────────────────────────────────────
function VideoPlayer({ lesson, color }: { lesson: Lesson; color: string }) {
  const totalSecs = (parseInt(lesson.duration) || 10) * 60;
  ...
  useEffect(() => {
    if (playing && !seeking) {
      intervalRef.current = setInterval(() => {
        setCurrent(c => {
          if (c >= totalSecs) { setPlaying(false); return totalSecs; }
          return c + speed;
        });
        setBuffered(b => Math.min(b + 0.4, 100));
      }, 1000);
    } else {
      if (intervalRef.current) clearInterval(intervalRef.current);
    }
    return () => { if (intervalRef.current) clearInterval(intervalRef.current); };
  }, [playing, seeking, speed, totalSecs]);

  const seek = (e: React.MouseEvent<HTMLDivElement>) => {
    const rect = barRef.current?.getBoundingClientRect();
    if (!rect) return;
    const ratio = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width));
    setCurrent(Math.floor(ratio * totalSecs));
  };

  const pct = (current / totalSecs) * 100;
  const SPEEDS = [0.5, 0.75, 1, 1.25, 1.5, 2];

  return (
    <div className="relative bg-black rounded-xl overflow-hidden group">
      {/* fake video canvas */}
      <div className="aspect-video flex flex-col items-center justify-center" style={{ background: `linear-gradient(135deg, #0a0a0a 0%, #111827 100%)` }}>
        ...
        {playing ? (
          ...
          <button onClick={() => setPlaying(true)}
            className="w-16 h-16 rounded-full bg-white/10 hover:bg-white/20 border-2 border-white/30 flex items-center justify-center transition-all hover:scale-110">
            <Play size={26} className="text-white ml-1" fill="white" />
          </button>
        )}
        {/* title overlay */}
        <p className="absolute bottom-14 left-4 text-white/60 text-[13px] font-medium drop-shadow">{lesson.title}</p>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent px-4 pb-3 pt-8 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="flex items-center gap-3">
          {/* prev/play/next */}
          <button className="text-white/70 hover:text-white transition-colors" onClick={() => setCurrent(c => Math.max(0, c - 10))}>
            <SkipBack size={16} />
          </button>
          <button className="text-white hover:scale-110 transition-transform" onClick={() => setPlaying(!playing)}>
            {playing ? <Pause size={20} fill="white" /> : <Play size={20} fill="white" />}
          </button>
          <button className="text-white/70 hover:text-white transition-colors" onClick={() => setCurrent(c => Math.min(totalSecs, c + 10))}>
            <SkipForward size={16} />
          </button>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-[#F2F5FC]">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-[820px] mx-auto px-6 py-6">
            ...
            {/* lesson content */}
            {activeLesson.type === "video" && <VideoPlayer lesson={activeLesson} color={course.color} />}
